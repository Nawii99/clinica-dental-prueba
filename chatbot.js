...
(function () {
  const RESPUESTAS = {
    ...
    precios:
      'Depende de cada caso, por eso la <strong>primera consulta es gratuita</strong>: te damos diagnóstico y presupuesto cerrado sin compromiso.<br><br>Como orientación: limpieza desde <strong>60 €</strong>, empaste desde <strong>65 €</strong>, blanqueamiento desde <strong>290 €</strong>, implante con corona desde <strong>1.100 €</strong> y ortodoncia invisible desde <strong>2.900 €</strong>. Financiación hasta 36 meses sin intereses.',
    urgencias:
    ...
    ubicacion:
      'Estamos en <strong>Calle de la Salud 12, 28001 Madrid</strong>.<br>Teléfono: <a href="tel:+34912345678">912 345 678</a><br>Email: <a href="mailto:hola@clinicasonrisa.es">hola@clinicasonrisa.es</a>',
    cita: null, // gestionada por el flujo de cita
  };

  const QUICK = [
    ['horarios', 'Horarios', 'i-clock'],
    ['servicios', 'Tratamientos', 'i-tooth'],
    ['precios', 'Precios', 'i-euro'],
    ['cita', 'Pedir cita', 'i-calendar'],
    ['urgencias', 'Urgencias', 'i-alert'],
    ['ubicacion', 'Dónde estamos', 'i-pin'],
  ];
  ...
  function pintarQuick() {
    quick.innerHTML = '';
    QUICK.forEach(([intencion, etiqueta, icono]) => {
      const b = document.createElement('button');
      b.type = 'button';
      b.innerHTML =
        '<svg class="ico" viewBox="0 0 24 24"><use href="#' + icono + '"/></svg>' + etiqueta;
      b.addEventListener('click', () => {
        burbuja(etiqueta, 'user');
        responder(intencion);
      });
      quick.appendChild(b);
    });
  ...
  function empezarCita() {
    pasoCita = 'nombre';
    datosCita = {};
    botDice('Perfecto. Te tomo los datos y te llamamos en menos de 24 h para confirmar.<br><br>¿Cómo te llamas?');
  }

  function responder(intencion) {
    if (intencion === 'cita') return empezarCita();
    if (intencion === 'saludo') {
      return botDice('¿En qué puedo ayudarte? Puedes usar los botones de abajo o escribirme directamente.');
    }
  ...
  function procesarTexto(texto) {
    ...
    if (pasoCita === 'telefono') {
      ...
      if (!/^\+?\d{9,15}$/.test(tel)) {
        return botDice('Ese teléfono no me cuadra. Escríbelo solo con números (por ejemplo: 600123456).');
      }
      datosCita.telefono = texto.trim();
      pasoCita = null;
      return botDice(
        'Listo, ' + datosCita.nombre + '. Hemos anotado tu solicitud y te llamaremos al <strong>' + datosCita.telefono +
